...
fitting - which is where `joints` finds a shell command's own `|`, `&&` and `;` for the permission
question to colour, and where `bullet` reads a list marker, because knowing the shape of the thing
being fitted is what fitting it takes), `tools/` (`fs.rs` for the filesystem tool, dispatching to `files.rs` for the three operations that
