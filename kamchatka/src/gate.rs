...
//!
//! note: Linux on x86_64 and aarch64, because a filter is a program written against one
//! architecture's system call numbers. Everywhere else [`holds`] is `false`, every function answers
//! `Unsupported`, and the shell falls back to reading the command for program names.

#![allow(unsafe_code)]

pub use imp::{Arriving, Attempt, Listener, hold, holds, pair};
    ...
    fn assemble(ops: &[Op]) -> Vec<libc::sock_filter> {
        ...
        for op in ops {
            ...
            code.push(match *op {
                Op::Mark(_) => continue,
                Op::Load(offset) => {
                    filter(libc::BPF_LD | libc::BPF_W | libc::BPF_ABS, 0, 0, offset)
                }
...
/// Where there is no gate, which is everywhere but Linux on these two architectures.
#[cfg(not(all(
    target_os = "linux",
    any(target_arch = "x86_64", target_arch = "aarch64")
)))]
mod imp {
    ...
    /// Nothing is held, because there is no gate to install.
    pub fn hold() -> io::Result<()> {
    ...
    /// There is no listener to send, so no pair to send it down.
    pub fn pair() -> io::Result<(std::process::Stdio, Arriving)> {
        Err(io::ErrorKind::Unsupported.into())
    }

    /// Where a listener would arrive; there is none of these here.
    pub struct Arriving(Infallible);

    impl Arriving {
        /// Unreachable: nothing makes one.
        pub async fn listener(self) -> io::Result<Option<Listener>> {
            match self.0 {}
        }
    }

    /// An attempt; there is none of these here.
    #[derive(Debug)]
    pub struct Attempt(Infallible);

    /// A listener; there is none of these here.
    pub struct Listener(Infallible);
    ...
    impl Listener {
        ...
        /// Unreachable: nothing makes one.
        pub fn answer(&self, _: Attempt, _: bool) {
            match self.0 {}
        }
    }
}
