//! What the program itself holds a command and its tools to, whether or not a kernel is helping.
//!
//! note: these are the claims that are true of `kamchatka` rather than of Landlock - the path
//! rules `fs` is held to, what a refusal names, the arguments a confinement travels as, and what
//! `shell` says about itself before anything runs. None of them spawns anything or asks the
//! kernel for a ruleset, so none of them has a reason to be Linux-only, and they were: the whole
//! suite was gated at the file, which left the `macos-latest` column in CI green while building
//! none of it. The mechanism is next door in `landlock.rs`, and that one is gated.
//!
//! note: `cfg(unix)` rather than nothing at all. The paths these are written against are `/usr`
//! and `/etc`, and a root without a drive letter is not absolute on Windows - so the rules would
//! be exercised with inputs that mean something else there. What Windows is owed is its own
//! suite, and this program has no confinement story on it to write one about.
...
use nachalnik::{Capability, Tool, Verdict};
