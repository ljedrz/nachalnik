...
//!
//! note: Linux-only, because Landlock is, and skipped rather than failed where it is not
//! available, which is what `Confinement::Unavailable` is for. A machine that cannot enforce this
//! should say so once, not fail a suite. What is *not* Linux-only is next door in `sandbox.rs`:
//! the path rules, the refusals and the argument round-trip are this program's own and run
//! wherever it does.
