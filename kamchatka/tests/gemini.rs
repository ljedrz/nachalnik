...
/// A chunk boundary is not a character boundary; see `streaming.rs` for the other half of this.
///
/// note: both providers assembled the stream by decoding each read as it arrived, lossily, so a
/// multi-byte character split between two of them became two replacement characters. This is the
/// dialect a model answering in Polish or Japanese is most likely to be reached through, since
/// `--gemini` is what carries a turn's order.
#[tokio::test]
